...
// The name matches the IdPRegistration mutating webhook convention so finalize can
// delete the secret via the idpregistration ownership label.
func SeedClientSecretName(alias string) string {
	return fmt.Sprintf("idpregistration-%s-client-secret", strings.TrimSpace(alias))
}
...
// ReservedSeedAliases returns aliases reserved for platform-managed seed providers.
func ReservedSeedAliases(cfg *SeedUpstreamConfig) map[string]struct{} {
	reserved := make(map[string]struct{})
	if cfg == nil {
		return reserved
	}
	for _, provider := range cfg.SeedUpstreamIdentityProviders.Providers {
		if alias := strings.TrimSpace(provider.Alias); alias != "" {
