...
func New(ctx context.Context, cfg *config.Config, mgr mcmanager.Manager, kcpClientGetter iclient.KCPClientGetter) (*subroutine, error) {
	...
	return &subroutine{
		...
		discoveryClient: util.NewSafeDiscoveryHTTPClient(time.Duration(cfg.HttpClientTimeoutSeconds) * time.Second),
		mgr:             mgr,
		cfg:             cfg,
...
func (s *subroutine) process(ctx context.Context, obj ctrlruntimeclient.Object) (subroutines.Result, error) {
	...
	if slices.Contains(s.cfg.IDP.RealmDenyList, realm) {
		msg := fmt.Sprintf("upstream IdP registration is not allowed for realm %q", realm)
		s.setObservedState(reg, realm, "", nil, false)
		return subroutines.Stop(msg), nil
	}

	adminClient := s.newAdminClient(realm)

	orgID, domains, err := s.reconcileBroker(ctx, adminClient, realm, reg)
	if err != nil {
		s.setObservedState(reg, realm, reg.Status.OrganizationID, reg.Status.LinkedEmailDomains, false)
		return subroutines.OK(), err
	}

	s.setObservedState(reg, realm, orgID, domains, true)
...
func (s *subroutine) finalize(ctx context.Context, obj ctrlruntimeclient.Object) (subroutines.Result, error) {
	...
	if alias != "" {
		current, err := adminClient.GetIdentityProvider(ctx, alias)
		if err != nil {
			return subroutines.OK(), err
		}
		if ownedByIdPRegistration(current) {
			...
			if reg.Status.OrganizationID != "" {
				if err := adminClient.DeleteOrganization(ctx, reg.Status.OrganizationID); err != nil {
					return subroutines.OK(), fmt.Errorf("deleting keycloak organization: %w", err)
				}
			}
		}
	}
	...
	if reg.Spec.OIDC != nil && reg.Spec.OIDC.ClientSecretRef.Name != "" {
		...
		if err := deleteManagedClientSecret(ctx, cluster.GetClient(), reg.Spec.OIDC.ClientSecretRef.Name, reg.Name, s.secretNamespace()); err != nil {
			return subroutines.OK(), fmt.Errorf("deleting client secret: %w", err)
		}
	}
...
func (s *subroutine) reconcileBroker(
	...
	if current != nil {
		if err := ensureManagedIdentityProvider(current, reg); err != nil {
			return "", nil, err
		}
	}
...
func (s *subroutine) newAdminClient(realmName string) *keycloak.AdminClient {
	return keycloak.NewAdminClient(s.adminHTTPClient, s.keycloakBaseURL, realmName)
}
...
func (s *subroutine) readClientSecret(ctx context.Context, secretName string) (string, error) {
	...
	if err := cluster.GetClient().Get(ctx, ctrlruntimeclient.ObjectKey{Name: secretName, Namespace: s.secretNamespace()}, secret); err != nil {
		return "", err
	}
...
func (s *subroutine) setObservedState(
	reg *pmcorev1alpha1.IdPRegistration,
	realm, orgID string,
	domains []string,
	configured bool,
	...
	if configured {
		reg.Status.RedirectURI = keycloak.BrokerRedirectURI(
			s.cfg.BaseDomain,
			s.cfg.Keycloak.PathPrefix,
			realm,
			reg.Spec.Alias,
		)
	} else {
...
func (s *subroutine) secretNamespace() string {
	if ns := strings.TrimSpace(s.cfg.IDP.IdPRegistrationSecretNamespace); ns != "" {
		return ns
	}
	return "default"
}

func ownedByIdPRegistration(idp *keycloak.IdentityProviderRepresentation) bool {
	if idp == nil || idp.Config == nil {
		return false
	}
	return idp.Config[keycloak.PlatformMeshManagedByKey] == keycloak.PlatformMeshManagedByIdPRegistration
}

func ensureManagedIdentityProvider(current *keycloak.IdentityProviderRepresentation, reg *pmcorev1alpha1.IdPRegistration) error {
	previouslyReady := meta.IsStatusConditionTrue(reg.Status.Conditions, conditions.ReadyCondition)
	if current.Config == nil {
		if !previouslyReady {
			return fmt.Errorf("identity provider alias %q already exists and is not managed by IdPRegistration", reg.Spec.Alias)
		}
		return nil
	}
	owner := current.Config[keycloak.PlatformMeshManagedByKey]
	if owner == keycloak.PlatformMeshManagedByIdPRegistration {
		return nil
	}
	if owner != "" {
		return fmt.Errorf("identity provider alias %q is managed by %q", reg.Spec.Alias, owner)
	}
	if !previouslyReady {
		return fmt.Errorf("identity provider alias %q already exists and is not managed by IdPRegistration", reg.Spec.Alias)
	}
