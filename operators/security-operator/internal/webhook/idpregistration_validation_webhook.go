...
func SetupIdPRegistrationValidatingWebhookWithManager(mgr ctrl.Manager, reservedAliases map[string]struct{}) error {
	return mcruntime.NewWebhookManagedBy(mgr, &pmcorev1alpha1.IdPRegistration{}).
		WithValidator(&idpRegistrationValidator{reservedAliases: reservedAliases}).
		Complete()
}
...
func (v *idpRegistrationValidator) ValidateCreate(_ context.Context, reg *pmcorev1alpha1.IdPRegistration) (admission.Warnings, error) {
	return nil, validateIdPRegistration(reg, v.reservedAliases)
}
...
func (v *idpRegistrationValidator) ValidateUpdate(_ context.Context, oldObj, newObj *pmcorev1alpha1.IdPRegistration) (admission.Warnings, error) {
	...
	return nil, validateIdPRegistration(newObj, v.reservedAliases)
}
...
func validateIdPRegistration(reg *pmcorev1alpha1.IdPRegistration, reservedAliases map[string]struct{}) error {
	spec := reg.Spec
	...
	if _, reserved := reservedAliases[alias]; reserved {
		return fmt.Errorf("alias %q is reserved for platform-managed upstream identity providers", alias)
	}
