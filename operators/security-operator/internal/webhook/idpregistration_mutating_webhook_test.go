...
func TestIdPRegistrationDefaulter_Default_persistsSecretAndSetsRef(t *testing.T) {
	...
	defaulter := &idpRegistrationDefaulter{resolver: &fakeClusterManager{client: cl}, secretNamespace: "default"}
...
func TestIdPRegistrationDefaulter_Default_rejectsForeignSecret(t *testing.T) {
	t.Parallel()
	...
	existing := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Labels: map[string]string{
				idpRegistrationSecretLabel: "other",
			},
		},
		Data: map[string][]byte{
			idpRegistrationSecretDataKey: []byte("old"),
	...
	err := defaulter.Default(context.Background(), reg)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "owned by IdPRegistration")
}

func TestIdPRegistrationDefaulter_Default_rejectsUnlabeledSecret(t *testing.T) {
	t.Parallel()
	...
	err := defaulter.Default(context.Background(), reg)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "not managed by IdPRegistration")
}
