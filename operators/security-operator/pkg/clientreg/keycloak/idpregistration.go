...
const (
	...
	// PlatformMeshManagedByKey marks Keycloak identity providers owned by IdPRegistration.
	PlatformMeshManagedByKey = "platformMeshManagedBy"
	// PlatformMeshManagedByIdPRegistration is the managed-by value for IdPRegistration brokers.
	PlatformMeshManagedByIdPRegistration = "idpregistration"
)
...
func ToKeycloakIdentityProviderFromRegistration(
	...
	rep.Config[PlatformMeshManagedByKey] = PlatformMeshManagedByIdPRegistration
...
func BrokerRedirectURI(baseDomain, keycloakPathPrefix, realm, alias string) string {
	...
	keycloakPathPrefix = strings.TrimSpace(keycloakPathPrefix)
	if keycloakPathPrefix == "" {
		keycloakPathPrefix = "/keycloak"
	}
	keycloakPathPrefix = "/" + strings.Trim(keycloakPathPrefix, "/")
	realm = strings.TrimSpace(realm)
	alias = strings.TrimSpace(alias)
	if baseDomain == "" || realm == "" || alias == "" {
		return ""
	}
	return fmt.Sprintf("https://%s%s/realms/%s/broker/%s/endpoint", baseDomain, keycloakPathPrefix, realm, alias)
}
