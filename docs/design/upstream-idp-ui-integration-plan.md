...
UI create/update
    ...
    security-operator reconcile
          |
          +-- (2) success -> status.conditions Ready=True, organizationId, linkedEmailDomains, redirectUri
          |
          +-- (2) Keycloak failure -> status.conditions Ready=False; IdPRegistration condition message=<error>
          |
          v
...
duplicate broker alias, Organizations API errors). Those appear in the
`IdPRegistration` condition message after reconcile (section 2).

### 2. Async feedback (Keycloak reconcile result)

**Mechanism:** security-operator lifecycle engine writes `status.conditions` after
each reconcile. The subroutine sets observed fields (`organizationId`,
`linkedEmailDomains`, `redirectUri`); success and failure are expressed only via
conditions.

| Status field / condition | UI use |
| --- | --- |
| `status.conditions` (`type=Ready`) | Primary success/failure indicator (`readyCondition`) |
| `status.conditions` (`type=IdPRegistration`) | **Show `.message` for Keycloak/operator errors** — contains actionable text (e.g. `failed to create identity provider: status 400 body: …`) |
...
| `status.redirectUri` | OAuth redirect URI to display on success |

On failure the subroutine returns an error; lifecycle sets the `IdPRegistration`
condition message from `err.Error()`. Keycloak Admin API bodies are included via
`readErrorResponse()` in the keycloak client.

**Optional backend improvements** (Phase 3 or parallel hardening):

| Improvement | Benefit |
| --- | --- |
| Parse Keycloak JSON error body into a shorter condition message | Cleaner UI text without raw HTTP bodies |
| Add `status.phase` (`Pending` / `Ready` / `Failed`) and `status.lastSyncTime` | Explicit wizard/list states (only if conditions prove insufficient) |
...
3. `status.conditions` (`type=IdPRegistration`, field `message`) — label **Keycloak status**, truncated in list

Detail view status group:

- aggregate `Ready` condition, `IdPRegistration` condition message,
  `status.organizationId`, `status.linkedEmailDomains`, `status.redirectUri`,
...
| Condition | UI |
| --- | --- |
| `Ready` condition `status=True` | Success — show linked domains / org id / redirect URI; offer “Test login” link |
| `Ready` condition `status=False` && `IdPRegistration` condition message non-empty | Failure — show subroutine condition message in error banner |
| User navigates away | Cancel waiter; list watch continues in background |

**Non-terminal** (keep waiting): `Ready` condition `status=Unknown` or `False`
with empty `IdPRegistration` condition message — reconcile still running or pending.
...
- `IdPRegistration` condition `message` (label **Keycloak status**, truncated) — see §2
...
- Status: aggregate `Ready` condition, `IdPRegistration` condition message,
  `organizationId`, `linkedEmailDomains`, `redirectUri`, full `conditions` — see §2–§3
...
| Owner creates IdP with bad client secret | CR stored; `Ready=False`, `IdPRegistration` condition message shows Keycloak error |
...
| Detail status prominence | Ensure `IdPRegistration` condition message is visible above the fold on detail view |
...
| Post-create navigation | Route to detail on successful mutation; show spinner then success/error from conditions |
...
| Keycloak error parsing | Friendlier `IdPRegistration` condition message (§2 optional improvements) |
...
| Raw Keycloak HTTP body in condition message | Phase 3: parse error JSON in operator (§2) |
...
- [ ] List shows aggregate `Ready` condition and Keycloak error in `IdPRegistration` condition message when reconcile fails
...
5. E2e smoke: admission rejection, async Keycloak error in conditions, list watch (§1–§3)
