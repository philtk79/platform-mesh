...
- `emailDomainRouting` (domains + autoRedirect / hideUntilDomainMatch) — clearing domains deletes the linked Keycloak organization; domain ownership is not verified yet (see below)
- `type: oidc`
- `oidc.clientId`
...
Clearing `emailDomainRouting.domains` or deleting the `IdPRegistration` removes the broker's Keycloak organization linkage and **deletes** the linked Keycloak organization. Avoid clearing domains if another registration might still rely on that organization.

**Domain ownership is not verified today.** Org owners can declare any domain string. A future release may require proof of domain control (DNS TXT, etc.) before linking.

## Alias collisions

Each org gets its own Keycloak realm, so two orgs cannot collide on an alias. Within a realm, a collision with a hand-created or platform-managed broker is detected at reconcile time via the `platformMeshManagedBy=idpregistration` provenance marker: the operator refuses to adopt a broker it does not own and reports it on `status.conditions`. Catching this at admission would mean a Keycloak Admin API call per request, which is not worth it. Seed writes are operator-internal and are not designed around admission.
